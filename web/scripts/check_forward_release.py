#!/usr/bin/env python3
"""Validate the production-based forward-only schema-101 source contract."""
...
def validate(root: Path = ROOT) -> list[str]:
    ...
    try:
        ...
        if not (schema['compatible_min'] <= 99 <= schema['compatible_max'] == schema['target'] == 101
                ...
                and release['migration_manifest'] == 'migrations-100-101.json'):
            errors.append('release must bridge production 099 to target 101 with rollback bounded at 099')
        manifest = json.loads((release_dir / 'migrations-100-101.json').read_text())
        ...
        if [entry['number'] for entry in entries] != [100, 101]:
            errors.append('forward manifest must contain ordered 100 then 101')
        for entry in entries:
            n = entry['number']
            canonical = ('100_correct_site_visit_counter_rpc.sql' if n == 100 else '101_add_variant_attribute_presentation.sql')
            if n not in (100,101) or entry['from_schema'] != n-1 or entry['to_schema'] != n or entry['compatibility'] != 'expand-only' or entry['path'] != 'web/database/migrations/' + canonical:
        ...
        for tree in ('database','supabase'):
            ...
            if versions != list(range(1,102)):
                errors.append('migration namespace must contain exactly 001 through 101: ' + tree)
