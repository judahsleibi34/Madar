...
do $$ declare actor integer; cid integer; oid uuid; earned integer; rule jsonb; ent uuid; result jsonb; conditions jsonb; begin
  select id into actor from public.users where tenant_id=971;
  select id into cid from public.users where tenant_id=972;
  for ent in select id from public.ecommerce_loyalty_entitlements where tenant_id=971 and customer_id=cid and status='active' loop
    perform public.revoke_ecommerce_loyalty_entitlement_safe(971,ent,actor);
  end loop;
  conditions:='[
    {"audience":"loyalty","product_ids":["97100000-0000-0000-0000-000000000001","97100000-0000-0000-0000-000000000002"],"discount_basis_points":3000,"validity_mode":"fixed_period","validity_days":1},
    {"audience":"loyalty","product_ids":["97100000-0000-0000-0000-000000000002"],"discount_basis_points":1500,"validity_mode":"lifetime","validity_days":null},
    {"audience":"normal","product_ids":["97100000-0000-0000-0000-000000000001"],"discount_basis_points":4000,"validity_mode":"lifetime","validity_days":null},
    {"audience":"normal","product_ids":["97100000-0000-0000-0000-000000000002"],"discount_basis_points":500,"validity_mode":"fixed_period","validity_days":1}
  ]'::jsonb;
  -- Foreign-tenant and malformed configurations fail atomically.
  begin
    perform public.save_ecommerce_loyalty_rule_v2_safe(972,cid,true,10000,5,conditions);
    raise exception 'cross_tenant_products_accepted';
  exception when sqlstate 'P0001' then
    if sqlerrm<>'ecommerce_loyalty_reward_product_invalid' then raise; end if;
  end;
  rule:=public.save_ecommerce_loyalty_rule_v2_safe(971,actor,true,10000,5,conditions);
  result:=public.best_ecommerce_product_discount_safe(971,null,'97100000-0000-0000-0000-000000000001');
  if (result->>'basis_points')::integer<>4000 or result->>'source'<>'normal' then raise exception 'guest_normal_offer_wrong'; end if;
  if (public.best_ecommerce_product_discount_safe(972,cid,'97100000-0000-0000-0000-000000000001')->>'basis_points')::integer<>0 then raise exception 'tenant_leak'; end if;
  select count(*) into earned from public.ecommerce_loyalty_transactions where transaction_type='earn';
  select id into oid from public.ecommerce_orders where tenant_id=971 and status='delivered' limit 1;
  perform public.process_ecommerce_loyalty_order_safe(971,oid);
  if (select count(*) from public.ecommerce_loyalty_transactions where transaction_type='earn')<>earned then raise exception 'edited_rule_reearned_order'; end if;
  result:=public.create_ecommerce_order_safe('{"tenant_id":971,"customer_name":"Normal","email":"normal@example.invalid","phone":"059","street":"Main Street","service_area_id":"95000000-0000-0000-0000-000000000001","payment_method":"cash_on_delivery","items":[{"product_id":"97100000-0000-0000-0000-000000000001","quantity":2}]}',repeat('6',64),repeat('6',64),repeat('6',64),cid);
  oid:=(result->'order'->>'id')::uuid;
  if (result->'order'->>'discount_total')::numeric<>80 or (result->'order'->>'total')::numeric<>120 then raise exception 'normal_checkout_total_wrong'; end if;
  perform public.transition_ecommerce_order_status_safe(971,oid,'confirmed',actor,'',repeat('1',64));
  perform public.transition_ecommerce_order_status_safe(971,oid,'preparing',actor,'',repeat('2',64));
  perform public.transition_ecommerce_order_status_safe(971,oid,'out_for_delivery',actor,'',repeat('3',64));
  perform public.transition_ecommerce_order_status_safe(971,oid,'delivered',actor,'',repeat('4',64));
  perform public.collect_ecommerce_cod_payment_safe(971,oid,actor);
  select id into ent from public.ecommerce_loyalty_entitlements where rule_id=(rule->>'id')::uuid and customer_id=cid;
  if ent is null or (select jsonb_array_length(reward_conditions) from public.ecommerce_loyalty_entitlements where id=ent)<>2 then raise exception 'multi_product_bundle_missing'; end if;
  if (select count(*) from public.ecommerce_loyalty_transactions where order_id=oid and transaction_type='reward_unlock')<>1 then raise exception 'bundle_spent_more_than_once'; end if;
  result:=public.best_ecommerce_product_discount_safe(971,cid,'97100000-0000-0000-0000-000000000001');
  if (result->>'basis_points')::integer<>4000 or result->>'source'<>'normal' or result->>'entitlement_id' is not null then raise exception 'normal_higher_not_selected'; end if;
  conditions:=jsonb_set(conditions,'{2,discount_basis_points}','2000');
  rule:=public.save_ecommerce_loyalty_rule_v2_safe(971,actor,true,10000,5,conditions);
  result:=public.best_ecommerce_product_discount_safe(971,cid,'97100000-0000-0000-0000-000000000001');
  if (result->>'basis_points')::integer<>3000 or result->>'source'<>'loyalty' then raise exception 'loyalty_higher_not_selected'; end if;
  result:=public.create_ecommerce_order_safe('{"tenant_id":971,"customer_name":"Reward","email":"normal@example.invalid","phone":"059","street":"Main Street","service_area_id":"95000000-0000-0000-0000-000000000001","payment_method":"cash_on_delivery","items":[{"product_id":"97100000-0000-0000-0000-000000000001","quantity":1},{"product_id":"97100000-0000-0000-0000-000000000002","variant_id":"97140000-0000-0000-0000-000000000001","quantity":2}]}',repeat('b',64),repeat('b',64),repeat('b',64),cid);
  if (result->'order'->>'discount_total')::numeric<>90 then raise exception 'multi_product_checkout_discount_wrong'; end if;
  if (select count(*) from public.ecommerce_order_items where order_id=(result->'order'->>'id')::uuid and loyalty_entitlement_id=ent and discount_source='loyalty')<>2 then raise exception 'multi_product_snapshots_wrong'; end if;
  update public.ecommerce_loyalty_entitlements set granted_at=now()-interval '2 days' where id=ent;
  update public.ecommerce_loyalty_rules set created_at=now()-interval '2 days' where id=(rule->>'id')::uuid;
  perform public.expire_ecommerce_loyalty_entitlements_safe(971,cid);
  if (select status from public.ecommerce_loyalty_entitlements where id=ent)<>'active' then raise exception 'lifetime_bundle_expired'; end if;
  result:=public.best_ecommerce_product_discount_safe(971,cid,'97100000-0000-0000-0000-000000000002');
  if (result->>'basis_points')::integer<>1500 then raise exception 'independent_condition_expiry_wrong'; end if;
  perform public.revoke_ecommerce_loyalty_entitlement_safe(971,ent,actor);
  perform public.revoke_ecommerce_loyalty_entitlement_safe(971,ent,actor);
  result:=public.best_ecommerce_product_discount_safe(971,cid,'97100000-0000-0000-0000-000000000002');
  if (result->>'basis_points')::integer<>0 then raise exception 'revoked_or_expired_offer_applied'; end if;
