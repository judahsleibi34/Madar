...
backup_path="$1"
: "${MADAR_SOURCE_DATABASE_ID:?MADAR_SOURCE_DATABASE_ID is required}"
: "${MADAR_RESTORE_DATABASE_ID:?MADAR_RESTORE_DATABASE_ID is required}"
: "${MADAR_RESTORE_PGHOST:?MADAR_RESTORE_PGHOST is required}"
: "${MADAR_RESTORE_PGPORT:?MADAR_RESTORE_PGPORT is required}"
: "${MADAR_RESTORE_PGUSER:?MADAR_RESTORE_PGUSER is required}"
: "${MADAR_RESTORE_PGPASSWORD:?MADAR_RESTORE_PGPASSWORD is required}"
: "${MADAR_RESTORE_PGDATABASE:?MADAR_RESTORE_PGDATABASE is required}"
[[ "$MADAR_SOURCE_DATABASE_ID" != "$MADAR_RESTORE_DATABASE_ID" ]] || die "restore target must differ from backup source"
[[ "${MADAR_RESTORE_CONFIRM_ISOLATED:-}" == "YES" ]] || die "set MADAR_RESTORE_CONFIRM_ISOLATED=YES after verifying the target is isolated"
case "$MADAR_RESTORE_PGHOST" in
...
PGHOST="$MADAR_RESTORE_PGHOST" \
PGPORT="$MADAR_RESTORE_PGPORT" \
PGUSER="$MADAR_RESTORE_PGUSER" \
PGPASSWORD="$MADAR_RESTORE_PGPASSWORD" \
PGDATABASE="$MADAR_RESTORE_PGDATABASE" \
PGSSLMODE="${MADAR_RESTORE_PGSSLMODE:-prefer}" \
pg_restore --exit-on-error --single-transaction \
  --dbname="$MADAR_RESTORE_PGDATABASE" --clean --if-exists --no-owner --no-acl \
  "${restore_list_args[@]}" \
