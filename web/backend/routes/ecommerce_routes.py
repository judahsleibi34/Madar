...
def get_catalog(request: Request, response: Response):
    ...
    try:
        tags = _rows(service_supabase.table("ecommerce_tags").select("*").eq("tenant_id", context.tenant_id).order("created_at", desc=True))
        categories = _rows(service_supabase.table("ecommerce_categories").select("*").eq("tenant_id", context.tenant_id).order("sort_order").order("created_at"))
        products = _rows(service_supabase.table("ecommerce_products").select("*").eq("tenant_id", context.tenant_id).order("created_at", desc=True))
        return {"tags": tags, "categories": categories, "products": _attach_product_tags(products, context.tenant_id)}
