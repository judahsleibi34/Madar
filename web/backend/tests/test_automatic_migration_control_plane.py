...
class AutomaticMigrationControlPlaneTests(unittest.TestCase):
    ...
    def test_current_manifest_and_rollback_contract_start_at_schema_96(self):
        manifest = json.loads(
            (
                WEB_ROOT
                / "deployment/releases"
        ...
        self.assertEqual(manifest["migrations"][-1]["to_schema"], 97)
        self.assertEqual(
            self.metadata["schema"]["rollback_compatible_max"], 96
        )
    ...
    def test_successful_96_to_97_records_target_only_after_worker_and_route_validation(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            state_root, operations, compatibility, events = self.fixture(root)
            backup = root / "backups/madar-20260830T000000Z"
            ...
            class FakeExecutor:
                ...
                def run(_self):
                    ...
                    events.append("migration:96->97")
                    operations.schema = 97
                    return {"status": "completed"}
        ...
        self.assertEqual(result["observed_schema"], 97)
        ...
        self.assertIn("migration:96->97", events)
    ...
    def test_prior_release_migrates_then_later_release_noops_idempotently(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            class TransitionExecutor:
                ...
                def run(_self):
                    events.append("previous-sql:96->97")
                    operations.schema = 97
        ...
        self.assertIn("previous-sql:96->97", events)
    ...
    def test_current_release_partial_state_without_backup_still_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (migration_state / "execution.json").write_text(json.dumps({
                ...
                "status": "running",
                "phase": "migration_97",
                "migrations": [{"number": 97, "status": "applying"}],
            }))
            with (
                patch.object(self.module, "_validate_stable_known_good"),
                patch.object(
    ...
    def test_backup_manifest_is_bound_to_release_and_source_schema(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest["database"]["schema_version"] = "96"
            (backup / "manifest.json").write_text(json.dumps(manifest))
            with self.assertRaisesRegex(
                RuntimeError, "automatic_migration_backup_identity_mismatch"
            ):
