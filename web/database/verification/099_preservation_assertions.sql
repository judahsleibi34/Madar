do $$ declare current_data jsonb; row record; begin
  for row in select * from public.rehearsal_099_snapshot loop
    execute format('select coalesce(jsonb_agg(to_jsonb(t) order by id),''[]'') from public.ecommerce_%I t',
      case row.kind when 'variants' then 'product_variants' when 'items' then 'order_items' when 'movements' then 'inventory_movements' else 'products' end) into current_data;
    -- Migration 097 adds snapshot columns; compare every pre-existing field.
    select coalesce(jsonb_agg((select jsonb_object_agg(key,value) from jsonb_each(element) where (row.data->0) ? key) order by element->>'id'),'[]')
    into current_data from jsonb_array_elements(current_data) element;
    if current_data is distinct from row.data then raise exception 'preservation_failed_%',row.kind; end if;
  end loop;
  if exists(select 1 from public.ecommerce_product_options where display_type <> 'text')
     or exists(select 1 from public.ecommerce_product_option_values where color_hex is not null) then raise exception '099_legacy_defaults_invalid'; end if;
  perform public.record_public_site_visit_safe(961,'website');
  perform public.record_public_site_visit_safe(961,'website');
  perform public.record_public_site_visit_safe(961,'store');
  if not exists(select 1 from public.site_visit_counters where tenant_id=961 and website_visits=2 and store_visits=1) then raise exception '098_counter_wrong'; end if;
  begin
  ...
  begin
    ...
    raise exception '099_invalid_hex_allowed';
...
do $$ declare attrs jsonb; opts jsonb; variants jsonb; before_variants jsonb; after_variants jsonb; begin
  ...
  if exists(select 1 from public.ecommerce_product_options where tenant_id=961 and display_type<>'color')
    or exists(select 1 from public.ecommerce_product_option_values where tenant_id=961 and color_hex is distinct from '#AABBCC') then raise exception '099_color_save_wrong'; end if;
  ...
  if before_variants is distinct from after_variants then raise exception '099_save_changed_variant_commercial_fields'; end if;
  begin
    ...
    raise exception '099_bad_swatch_allowed';
  exception when sqlstate 'P0001' then if sqlerrm<>'ecommerce_color_swatch_required' then raise; end if; end;
  if exists(select 1 from public.ecommerce_product_option_values where tenant_id=961 and color_hex is distinct from '#AABBCC') then raise exception '099_invalid_save_not_atomic'; end if;
end $$;
