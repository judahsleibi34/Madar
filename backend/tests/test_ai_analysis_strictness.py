...
def test_blocks_raw_file_reading_before_provider_call():
    ...
    assert "raw file contents" in result["message"]
    ask_planner.assert_not_called()


def test_blocks_full_dataset_dump_before_provider_call():
    with patch.object(ai_service, "ask_planner") as ask_planner:
...
def test_blocks_sensitive_value_lookup_before_provider_call():
    with patch.object(ai_service, "ask_planner") as ask_planner:
...
def test_blocks_chart_requests_before_provider_call():
    with patch.object(ai_service, "ask_planner") as ask_planner:
        result = _run_ai("Create a bar chart for score by department.")

    assert result["success"] is False
    assert result["type"] == "blocked"
    assert "charts" in result["message"]
    ask_planner.assert_not_called()


def test_safe_aggregate_request_still_reaches_provider():
    planner_response = {
        "intent": "chat",
        "safety": "safe",
...
def test_planner_chart_action_is_rejected():
    response = {
        "intent": "analysis",
        "safety": "safe",
        "mode": "predefined",
    ...
    with pytest.raises(PlanValidationError):
        validate_and_normalize_planner_response(response, allowed_columns=["department", "score"])


def test_generated_code_cannot_plot():
    code = """
import pandas as pd
result = {
    "title": "Plot",
...
def test_free_plan_defaults_to_five_daily_questions_and_output_cap():
    summary = get_ai_runtime_summary("free")

    assert summary["limits"]["daily_messages"] == 5
    assert summary["max_output_tokens"] == 900
