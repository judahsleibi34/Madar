...
def reserve_storage(*, tenant_id: int, user_id: int | None, category: str, size_bytes: int, storage_root: Path, client=None) -> str:
    ...
    try:
        response = database_client.rpc("reserve_storage_bytes", {
            ...
            "p_tenant_quota": _env_bytes("TENANT_STORAGE_QUOTA_BYTES", DEFAULT_TENANT_QUOTA_BYTES),
            "p_user_quota": _env_bytes("USER_STORAGE_QUOTA_BYTES", DEFAULT_USER_QUOTA_BYTES) if user_id is not None else None,
        }).execute()
...
def get_tenant_storage_usage(tenant_id: int, *, client=None) -> dict[str, Any]:
    ...
    ).eq("tenant_id", int(tenant_id)).eq("scope_key", "tenant").limit(1).execute()
    rows = getattr(response, "data", None) or []
    if not rows:
        return {"used_bytes": 0, "reserved_bytes": 0, "quota_bytes": _env_bytes("TENANT_STORAGE_QUOTA_BYTES", DEFAULT_TENANT_QUOTA_BYTES)}
    row = rows[0]
    total = int(row.get("used_bytes") or 0) + int(row.get("reserved_bytes") or 0)
    quota = int(row.get("quota_bytes") or 1)
    return {**row, "near_capacity": total >= int(quota * 0.9)}
