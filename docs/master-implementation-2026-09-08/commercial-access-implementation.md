...
- Fresh PostgreSQL migration/ledger suite through schema99:14 tests, exact RLS/grants, SQL concurrency/idempotency and synthetic ledger dump/restore passed with zero invalid indexes.
...
- Pinned pip-audit2.10.1 and the shipped npm audit report no known vulnerabilities. No approved image scanner is installed; image vulnerability scan is not claimed.
- All52 direct/aliased privileged-client application files are classified and rechecked on2026-09-14. Ordinary-user privileged paths retain explicit user, membership, tenant, resource, role and capability boundaries. Static inventory is an additional completeness gate, not proof that RLS constrains service-role access.
